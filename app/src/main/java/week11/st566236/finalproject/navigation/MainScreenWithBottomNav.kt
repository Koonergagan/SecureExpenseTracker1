package week11.st566236.finalproject.navigation

import androidx.compose.foundation.layout.padding
import androidx.compose.material3.Icon
import androidx.compose.material3.Scaffold
import androidx.compose.material3.Text
import androidx.compose.runtime.Composable
import androidx.compose.runtime.getValue
import androidx.compose.ui.Modifier
import androidx.navigation.compose.NavHost
import androidx.navigation.compose.composable
import androidx.navigation.compose.currentBackStackEntryAsState
import androidx.navigation.compose.rememberNavController
import week11.st566236.finalproject.ui.analytics.AnalyticsScreen
import week11.st566236.finalproject.ui.components.BottomNavScreen
import week11.st566236.finalproject.ui.components.bottomNavItems
import week11.st566236.finalproject.ui.expenses.ExpensesScreen
import week11.st566236.finalproject.ui.home.HomeScreen
import androidx.compose.material3.NavigationBar
import androidx.compose.material3.NavigationBarItem
import androidx.navigation.NavController

@Composable
fun MainScreenWithBottomNav(appNavController: NavController) {
    // Create a local NavController for managing bottom navigation within this screen
    val navController = rememberNavController()

    Scaffold(
        bottomBar = {

            // Bottom navigation bar
            NavigationBar {
                // Observe the current back stack entry to highlight the selected nav item
                val navBackStackEntry by navController.currentBackStackEntryAsState()
                val currentRoute = navBackStackEntry?.destination?.route

                // Iterate through all bottom nav items and create NavigationBarItem for each
                bottomNavItems.forEach { screen ->
                    NavigationBarItem(
                        icon = { Icon(screen.icon, contentDescription = screen.label) },
                        label = { Text(screen.label) },
                        selected = currentRoute == screen.route,
                        onClick = {
                            // Navigate to the selected route
                            navController.navigate(screen.route) {
                                launchSingleTop = true // Prevent multiple copies of the same destination
                                restoreState = true // Restore previous state if exists
                                popUpTo(navController.graph.startDestinationId) { saveState = true } // Pop to start destination and save state
                            }
                        }
                    )
                }
            }
        }
    ) { innerPadding ->
        // NavHost manages navigation between composable destinations within this bottom nav
        NavHost(
            navController = navController,
            startDestination = BottomNavScreen.Home.route,
            modifier = Modifier.padding(innerPadding)
        ) {
            // HomeScreen receives the main app navController for nested navigation
            composable(BottomNavScreen.Home.route) { HomeScreen(navController = appNavController) }
            // ExpensesScreen does not need the appNavController
            composable(BottomNavScreen.Expenses.route) { ExpensesScreen() }
            // AnalyticsScreen shows charts and data summaries
            composable(BottomNavScreen.Analytics.route) { AnalyticsScreen() }
        }
    }
}